...
const mapApiData = (item) => {
  ...
  return {
    ...
    irradiation: isValidValue(item.irradiation) ? Number(item.irradiation) : Math.random() * 1000 + 200, // Fake irradiation 200-1200 W/m²
...
const Station1 = () => {
  ...
  const fetchData = async () => {
    setError(null);
    try {
      console.log(`[Kalimantan Station1] Fetching ${filter} data from fake API...`);
      let result = [];
      
      // Dapatkan data berdasarkan filter
      if (filter === '1d') {
        result = await apiClient.getData('kalimantan');
      } else if (filter === '7d') {
        result = await apiClient.getHistoricalData('kalimantan', 7);
      } else if (filter === '1m') {
        result = await apiClient.getHistoricalData('kalimantan', 30);
      }
      
      console.log("Kalimantan Fake API Response:", result);
      
      const mapped = Array.isArray(result) ? result.map(mapApiData) : [];
      mapped.sort((a, b) => {
      ...
      setAllData([]);
      console.error("Kalimantan fake API error:", err);
    }
  };
